...
export default class TddetectiveView {
  ...
  updateView(aClassName, classRange, editor) {
    this.highlightText(classRange, editor);
    this.playSound();
    // this.showNotification(aClassName);
  }

  highlightText(classRange, editor) {
  ...
  showNotification(aClassName) {
    atom.notifications.addWarning("Missing spec file for " + aClassName)
